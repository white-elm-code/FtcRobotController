...
/*
 * This is the tank4auto class. It implements the DrivetrainAuto interface and is used for a 4 wheeled tank drivetrain.
 */
public class tank4auto implements DrivetrainAuto {
    ...
    @Override
    public void drive(double distance, DISTANCE_UNITS units, Constants.DIRECTION direction, double velocity) {
        reset();
        switch (direction){
            case FORWARD: forward(distance, units); break;
            case BACKWARD: backward(distance, units); break;
            case ROTATE_LEFT: rotateLeft(distance); break;
            case ROTATE_RIGHT: rotateRight(distance); break;
            default: break;
        }
        runToPosition();
    ...
    @Override
    public void stop () {
        front_left_drive.setPower(0.0);
        back_left_drive.setPower(0.0);
        front_right_drive.setPower(0.0);
        back_right_drive.setPower(0.0);
    }
    /*
     * These are the tank4auto class's private helper functions.
    ...
    private void rotateLeft (double degrees) {
    ...
    private void rotateRight (double degrees) {
        int ticks = (int) ((TICKS_PER_360_ROTATION * degrees) / 360.0);
        front_left_drive.setTargetPosition(-ticks);
        back_left_drive.setTargetPosition(-ticks);
        front_right_drive.setTargetPosition(ticks);
